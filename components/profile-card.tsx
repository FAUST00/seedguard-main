 ...
 * every stacking context (glass-effect backdrop-filter, transform animations, etc).
 */
...
export function ProfileCard({ entry, children }: Props) {
  ...
  // Guard against SSR — portal needs document.body
  useEffect(() => { setMounted(true); }, []);

  // Compute fixed position relative to trigger each time the card opens
  useEffect(() => {
    if (!open || !triggerRef.current) return;
    const rect = triggerRef.current.getBoundingClientRect();
    const gap = 12;
    let left = rect.right + gap;
    // Flip to left side if card would overflow the viewport
  ...
  const popup = (
    <div
      ...
      className="fixed z-[9999] glass-effect border border-primary/30 rounded-2xl p-5 shadow-2xl animate-scale-in"
    >
      {/* Avatar + name */}
